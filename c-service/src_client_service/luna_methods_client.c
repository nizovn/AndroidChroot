...
// Called directly from webOS, and returns directly to webOS.
//
bool client_method(LSHandle* lshandle, LSMessage *message, void *ctx) {
	LSError lserror;
	LSErrorInit(&lserror);
...
// Extract the id argument from the message
	...
	bool mount=WEXITSTATUS(system(APP_PATH "/c-service/androidchroot_client"));
	sprintf(reply, "{\"returnValue\": true, \"mounted\": \"%s\"}",(mount==0)?"true":"false");
